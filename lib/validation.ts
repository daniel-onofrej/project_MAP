import type { AgentConfig, ConflictRule, NodeData, NodeType } from './types';

const PASSIVE_NODE_TYPES: NodeType[] = [
  'PERSONA',
  'INPUT',
  'TRIGGER',
  'CONFIG',
...
function getPromptLines(nodes: NodeData[], nodeIds: string[]): { start: number; end: number }[] | undefined {
...
export function validateAgentConfig(config: AgentConfig): ConflictRule[] {
  ...
  config.connections.forEach(conn => {
    ...
    if (sourceNode && PASSIVE_NODE_TYPES.includes(sourceNode.type)) return;
    hasIncoming.add(conn.target);
  });

  const startNodes = config.nodes.filter(
    node => (node.type === 'START' || node.type === 'AGENT') && !hasIncoming.has(node.id)
  );
  ...
  const endNodes = config.nodes.filter(
  ...
  config.connections.forEach(conn => {
    ...
    if (sourceNode && PASSIVE_NODE_TYPES.includes(sourceNode.type)) return;
    if (targetNode && PASSIVE_NODE_TYPES.includes(targetNode.type)) return;
  ...
  // DAG Rule: Topological sortability
  {
    ...
    config.connections.forEach(conn => {
      ...
      if (!adjList.has(conn.source)) adjList.set(conn.source, []);
      adjList.get(conn.source)!.push(conn.target);
    });

    const queue: string[] = [];
    kahnInDegree.forEach((deg, id) => { if (deg === 0) queue.push(id); });
  ...
  // INFO: Handshaking lemma sanity check
  {
    ...
    const validEdges = config.connections.filter(conn => {
      ...
      const sourceNode = nodeMap.get(conn.source);
      const targetNode = nodeMap.get(conn.target);
      if (sourceNode && PASSIVE_NODE_TYPES.includes(sourceNode.type)) return false;
      if (targetNode && PASSIVE_NODE_TYPES.includes(targetNode.type)) return false;
      return true;
    }).length;
...
function findCircularDependencies(config: AgentConfig): string[][] {

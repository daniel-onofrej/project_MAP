...
const EXECUTABLE_TYPES = ['AGENT', 'TASK', 'ACTION', 'STEP', 'TOOL', 'RESOLUTION'];
const PASSTHROUGH_TYPES = ['START', 'RULE', 'CONFIG', 'PERSONA', 'REFERENCE', 'TRIGGER', 'LOGGING', 'INPUT', 'OPTION'];
/** Diff two JSON strings and return DataChange[] */
function computeDataChanges(inputStr, outputStr) {
    const changes = [];
    try {
...
export class AgentRunner {
    ...
    initialInput;
    memory = new Map();
    constructor(agent, initialInput) {
        this.agent = agent;
    ...
    preFlightCheck() {
        ...
        const deadEnds = this.agent.nodes.filter(n => !hasOutgoing.has(n.id) && n.type !== 'END' && n.type !== 'HANDOFF');
        ...
        for (const node of guardNodes) {
            const escalation = this.agent.connections.find(c => c.source === node.id && c.type === 'escalation');
            if (!escalation) {
                issues.push({
                    severity: 'warning',
    ...
    async executeNode(node, input, stepRef) {
        ...
        if (node.type === 'END') {
            ...
            if (validEnumValues.length === 0)
                validEnumValues.push('default_path');
            ...
            const matchedConn = outgoing.find(c => (c.condition ?? c.target).toLowerCase() === chosenLower ||
                (c.condition ?? c.target).toLowerCase().includes(chosenLower) ||
                chosenLower.includes((c.condition ?? '').toLowerCase())) ?? outgoing[0];
            ...
            if (!matchedConn || matchedConn === outgoing[0]) {
                const exactMatch = outgoing.find(c => (c.condition ?? c.target).toLowerCase() === chosenLower);
            ...
            if (blocked) {
                const escalationConn = this.agent.connections.find(c => c.source === node.id && c.type === 'escalation');

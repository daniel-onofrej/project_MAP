export function calculateComplexity(agent) {
    ...
    const outgoingCounts = nodes.map(node => connections.filter(c => c.source === node.id).length);
    ...
    const orphanedNodes = nodes.filter(node => !connections.some(c => c.source === node.id || c.target === node.id) && node.type !== 'AGENT');
...
export function calculateSimplicityScore(agent) {
    // Gather all text: originalPrompt + node descriptions + logicSnippets
    const textParts = [];
    if (agent.originalPrompt)
        textParts.push(agent.originalPrompt);
    for (const node of agent.nodes) {
        if (node.description)
            textParts.push(node.description);
        if (node.config?.logicSnippet)
            textParts.push(node.config.logicSnippet);
    }
...
function calculateMaxDepth(nodes, connections) {
    if (nodes.length === 0)
        return 0;
    // Find start node

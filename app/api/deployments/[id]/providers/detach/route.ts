...
export async function POST(request: NextRequest, { params }: { params: Promise<Params> }) {
  ...
  const worker = await callDeploymentWorker(`/deployments/${id}/providers/detach`, {
    method: 'POST',
  ...
  await db.update(deploymentProviders)
    .set({ attachStatus: 'detached', lastVerifiedAt: new Date() })
  ...
  await addDeploymentEvent({
    deploymentId: id,
    eventType: 'provider_detached',
    message: `Provider ${providerName} detached.`,
